...
const createMockStats = (isFile: boolean, isDirectory: boolean): fs.Stats => {
    // Use Partial<fs.Stats> and cast or create a class implementing fs.Stats
    const stats = new fs.Stats(); // Use actual Stats constructor
    stats.isFile = () => isFile;
    stats.isDirectory = () => isDirectory;
    // Add other properties/methods if needed by the code under test (e.g., size)
    stats.size = isFile ? 100 : 0; // Example size
    stats.mtime = new Date();
    // ... mock other fs.Stats properties/methods as needed
    return stats;
};
...
describe('processPath', () => {
  ...
  let baseOptions: Omit<ProcessPathOptions, 'writer' | 'debug' | 'stats' | 'mainIg'>;
  ...
  beforeEach(() => {
    ...
    mockPrintPath.mockResolvedValue(); // Default mock implementation
    ...
    (fsp.stat as jest.Mock).mockImplementation(async (p: string) => {
        ...
        if (p.endsWith('.txt') || p.endsWith('.js') || p.endsWith('.ts') || p.endsWith('.log') || p.endsWith('.tmp') || BINARY_FILE_EXTENSIONS.some(ext => p.toLowerCase().endsWith(ext))) {
        ...
        if (p === baseProjectPath || p.includes('src') || p.includes('node_modules') || p.includes('build') || p.includes('.git')) {
  ...
  const runProcessPath = (targetPathRelative: string, optionsOverrides: Partial<ProcessPathOptions> = {}) => {
    ...
    const absoluteTargetPath = path.resolve(options.baseIgnorePath, targetPathRelative);
  ...
  describe('File Handling', () => {
    ...
    test('should process a basic file', async () => {
      ...
      expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Processing path: ${absoluteFilePath}`));
      expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Reading file: ${absoluteFilePath}`));
      expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Printing file: ${absoluteFilePath}`));
    });
    ...
    test('should skip file due to extension mismatch', async () => {
        ...
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`Skipping file (ext mismatch): ${path.basename(absoluteFilePath)}`));
    });
     ...
     test('should process file with matching extension', async () => {
        ...
        expect(mockStatsCounter.foundFiles).toBe(1);
        expect(mockStatsCounter.skippedFiles).toBe(0);
        expect(mockDebug).toHaveBeenCalledWith(chalk.green(`File passed extension filter: ${path.basename(absoluteFilePath)}`));
    });

    test('should skip hidden file by default', async () => {
        const hiddenFilePathRelative = '.hiddenfile.txt';
        const absoluteHiddenPath = path.resolve(baseProjectPath, hiddenFilePathRelative);
        // Mock stat specifically for this hidden file path
        (fsp.stat as jest.Mock).mockResolvedValueOnce(createMockStats(true, false));
        ...
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`    Skipping hidden: ${path.basename(absoluteHiddenPath)}`));
    });
    ...
    test('should include hidden file when includeHidden is true', async () => {
        ...
        expect(mockPrintPath).toHaveBeenCalledWith(mockWriter, absoluteHiddenPath, 'Hidden content', false, false, false);
        expect(mockStatsCounter.foundFiles).toBe(1);
        expect(mockStatsCounter.skippedFiles).toBe(0);
    });
    ...
    test('should skip file via .gitignore pattern', async () => {
        ...
        expect(mockDebug).toHaveBeenCalledWith(expect.stringContaining(`Skipping due to ignore rules: ${path.basename(absoluteLogPath)}`));
    });

    test('should skip file via options.ignorePatterns', async () => {
        const tempFilePathRelative = 'temp.tmp';
        const absoluteTempPath = path.resolve(baseProjectPath, tempFilePathRelative);
        (fsp.stat as jest.Mock).mockResolvedValueOnce(createMockStats(true, false));
        ...
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`Skipping file due to --ignore pattern: ${path.basename(absoluteTempPath)}`));
    });

    test('should skip binary file by default', async () => {
        const binaryFilePathRelative = 'image.png'; // .png is in BINARY_FILE_EXTENSIONS
        const absoluteBinaryPath = path.resolve(baseProjectPath, binaryFilePathRelative);
        (fsp.stat as jest.Mock).mockResolvedValueOnce(createMockStats(true, false));
        ...
        expect(mockDebug).toHaveBeenCalledWith(expect.stringContaining(`Skipping binary file: ${path.basename(absoluteBinaryPath)}`));
    });
    ...
    test('should include binary file when includeBinaryFiles is true', async () => {
        ...
        await runProcessPath(binaryFilePathRelative, { includeBinaryFiles: true });
        expect(mockPrintPath).toHaveBeenCalledWith(mockWriter, absoluteBinaryPath, 'binary content', false, false, false);
        expect(mockStatsCounter.foundFiles).toBe(1);
        expect(mockStatsCounter.skippedFiles).toBe(0);
    });
    ...
    test('should handle file read error gracefully', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(chalk.yellow(`Warning: Skipping file ${absoluteFilePath} due to read error: ${error.message}`));
     ...
     test('should handle stat error gracefully', async () => {
        ...
        expect(mockPrintPath).not.toHaveBeenCalled();
        expect(consoleErrorSpy).toHaveBeenCalledWith(chalk.red(`Error accessing path ${absoluteFilePath}: ${error.message}`));
        expect(mockStatsCounter.foundFiles).toBe(0);
  ...
  describe('Directory Handling', () => {
     ...
     const absoluteChildFilePath = path.resolve(baseProjectPath, childFilePathRelative);
     const hiddenDirName = '.git';
     const hiddenDirPathRelative = hiddenDirName;
     const absoluteHiddenDirPath = path.resolve(baseProjectPath, hiddenDirPathRelative);
     ...
     beforeEach(() => {
        ...
        (fsp.stat as jest.Mock).mockImplementation(async (p: string) => {
            ...
            if (p === path.resolve(absoluteHiddenDirPath, 'config')) return createMockStats(true, false);
            // Add stat mock for ignored dirs/files if needed
            if (p === path.resolve(baseProjectPath, 'node_modules')) return createMockStats(false, true);
            if (p === path.resolve(baseProjectPath, 'build')) return createMockStats(false, true);
            if (p === path.resolve(baseProjectPath, 'build', 'output.log')) return createMockStats(true, false);
        ...
        (fsp.readdir as jest.Mock).mockImplementation(async (p: string, options?: { withFileTypes: boolean }) => {
        ...
        (fsp.readFile as jest.Mock).mockImplementation(async (p: string) => {
            ...
            if (p === path.resolve(absoluteHiddenDirPath, 'config')) return 'Hidden config content';
            if (p === path.resolve(baseProjectPath, 'build', 'output.log')) return 'Build log content';
    ...
    test('should process directory and recurse', async () => {
        ...
        expect(fsp.readdir).toHaveBeenCalledWith(absoluteDirPath, { withFileTypes: true });
        expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Processing path: ${absoluteDirPath}`));
        expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Path is a directory. Reading entries...`));
        expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Found 1 entries in ${absoluteDirPath}`)); // Based on mock readdir
        ...
        expect(mockPrintPath).toHaveBeenCalledWith(mockWriter, absoluteChildFilePath, 'Child file content', false, false, false);
    ...
    test('should skip hidden directory by default', async () => {
        ...
        expect(fsp.readdir).not.toHaveBeenCalledWith(absoluteHiddenDirPath, expect.anything());
        expect(mockPrintPath).not.toHaveBeenCalled();
        expect(mockStatsCounter.foundFiles).toBe(0);
        expect(mockStatsCounter.skippedFiles).toBe(0);
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`    Skipping hidden: ${path.basename(absoluteHiddenDirPath)}`));
    });
    ...
    test('should include hidden directory when includeHidden is true', async () => {
        ...
        expect(fsp.stat).toHaveBeenCalledWith(absoluteHiddenDirPath);
        expect(fsp.readdir).toHaveBeenCalledWith(absoluteHiddenDirPath, { withFileTypes: true });
        // Should process the child file 'config' inside the hidden dir
        const absoluteHiddenChildPath = path.resolve(absoluteHiddenDirPath, 'config');
        expect(fsp.stat).toHaveBeenCalledWith(absoluteHiddenChildPath);
        expect(mockPrintPath).toHaveBeenCalledWith(mockWriter, absoluteHiddenChildPath, 'Hidden config content', false, false, false);
        expect(mockStatsCounter.foundFiles).toBe(1);
        expect(mockStatsCounter.skippedFiles).toBe(0);
        expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Processing path: ${absoluteHiddenDirPath}`));
    });
    ...
    test('should skip directory via .gitignore pattern', async () => {
        ...
        const absoluteIgnoredDirPath = path.resolve(baseProjectPath, ignoredDirPathRelative);
        ...
        expect(fsp.readdir).not.toHaveBeenCalledWith(absoluteIgnoredDirPath, expect.anything());
        expect(mockPrintPath).not.toHaveBeenCalled();
        expect(mockStatsCounter.foundFiles).toBe(0);
        expect(mockStatsCounter.skippedFiles).toBe(0);
        expect(mockDebug).toHaveBeenCalledWith(expect.stringContaining(`Skipping due to ignore rules: ${path.basename(absoluteIgnoredDirPath)}`));
    });
    ...
    test('should skip directory via options.ignorePatterns when ignoreFilesOnly is false', async () => {
        ...
        const absoluteBuildDirPath = path.resolve(baseProjectPath, buildDirPathRelative);

        await runProcessPath(buildDirPathRelative, { ignorePatterns: ['build/'], ignoreFilesOnly: false });

        expect(fsp.stat).toHaveBeenCalledWith(absoluteBuildDirPath);
        expect(fsp.readdir).not.toHaveBeenCalledWith(absoluteBuildDirPath, expect.anything());
        expect(mockPrintPath).not.toHaveBeenCalled();
        expect(mockStatsCounter.foundFiles).toBe(0);
        expect(mockStatsCounter.skippedFiles).toBe(0);
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`Skipping directory due to --ignore pattern: ${path.basename(absoluteBuildDirPath)}`));
    });
    ...
    test('should NOT skip directory via options.ignorePatterns when ignoreFilesOnly is true, but skip files inside', async () => {
        ...
        const absoluteFileInBuildPath = path.resolve(baseProjectPath, fileInBuildPathRelative);

        await runProcessPath(buildDirPathRelative, { ignorePatterns: ['build/*'], ignoreFilesOnly: true });

        // Should still process the directory itself
        expect(fsp.stat).toHaveBeenCalledWith(absoluteBuildDirPath);
        expect(fsp.readdir).toHaveBeenCalledWith(absoluteBuildDirPath, { withFileTypes: true });
        expect(mockDebug).toHaveBeenCalledWith(chalk.cyan(`Processing path: ${absoluteBuildDirPath}`));
        ...
        expect(mockDebug).toHaveBeenCalledWith(chalk.yellow(`Skipping file due to --ignore pattern: ${path.basename(absoluteFileInBuildPath)}`));
    });

    test('should handle directory read error gracefully', async () => {
        const error: NodeJS.ErrnoException = new Error('Readdir permission denied');
        error.code = 'EACCES';
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(chalk.red(`Error reading directory ${absoluteDirPath}: ${error.message}`));
     ...
     test('should sort directory entries before processing', async () => {
        ...
        (fsp.stat as jest.Mock).mockImplementation(async (p: string) => {
            if (p === absoluteDirPath) return createMockStats(false, true);
            if (p === path.resolve(absoluteDirPath, fileA)) return createMockStats(true, false);
            if (p === path.resolve(absoluteDirPath, fileB)) return createMockStats(true, false);
            if (p === path.resolve(absoluteDirPath, subDirC)) return createMockStats(false, true);
             if (p === path.resolve(absoluteDirPath, subDirC, 'd.txt')) return createMockStats(true, false); // File in subdir
        ...
        (fsp.readdir as jest.Mock).mockImplementation(async (p: string, options?: { withFileTypes: boolean }) => {
            if (p === absoluteDirPath) {
             ...
             if (p === path.resolve(absoluteDirPath, subDirC)) {
                 // Content of subdir
                 return options?.withFileTypes ? [createMockDirent({ name: 'd.txt', isFile: true })] : ['d.txt'];
             }
            return [];
        });
        ...
        mockDebug.mockImplementation((msg: string) => {
            if (msg.startsWith(chalk.cyan(`Processing path:`))) {
                processPathCalls.push(msg.split(': ')[1]);
            }
        });
  ...
  describe('Option Propagation and Formatting', () => {
    ...
    test('should pass formatting options down during recursion', async () => {
     ...
     test('should use markdown formatting when specified', async () => {
        await runProcessPath(dirPathRelative, { markdown: true, claudeXml: false });
